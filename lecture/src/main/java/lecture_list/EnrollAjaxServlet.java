		...
		if (enrolledCourses != null && !enrolledCourses.isEmpty()) {
			for (Map.Entry<Integer, Integer> entry : enrolledCourses.entrySet()) {
				...
				if (selectedClass.getDayOfWeek().equals(enrolledClass.getDayOfWeek())) {
					if (!(Integer.parseInt(selectedClass.getEndTime()) < Integer.parseInt(enrolledClass.getStartTime())
							|| Integer.parseInt(selectedClass.getStartTime()) > Integer
									.parseInt(enrolledClass.getEndTime()))) {
						JsonObject jsonResponse = new JsonObject();
